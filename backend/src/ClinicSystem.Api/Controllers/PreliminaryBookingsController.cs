...
public sealed class PreliminaryBookingsController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            if (!await _doctorScope.CanAccessAllAsync(
                    actorUserId,
                    roles,
                    [requestedDoctorId],
                    cancellationToken))
        ...
        {
            var currentScope = await ResolveScopeAsync(cancellationToken);
            ...
            if (currentScope.Count != 1)
            {
                return BadRequest(new
                {
                    ...
                    message = "Select the responsible doctor before saving the preliminary booking."
                });
            }

            targetDoctorId = currentScope.Single();
        }
