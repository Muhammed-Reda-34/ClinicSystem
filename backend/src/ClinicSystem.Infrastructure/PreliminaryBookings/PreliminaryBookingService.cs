...
public sealed class PreliminaryBookingService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await _db.PreliminaryBookings
            ...
            .Where(x =>
                ...
                && x.DoctorId != null
                && allowedDoctorIds.Contains(x.DoctorId.Value)
                && x.PatientId != null
                && x.Patient != null
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Backfill the doctor for legacy Phase 09-12 bookings.
        // New bookings never depend on this repair path.
        var legacyDoctorRows = await _db.PreliminaryBookings
            .Include(x => x.Patient)
            .ThenInclude(x => x!.DoctorAssignments)
            .Where(x =>
                x.DoctorId == null
        ...
        foreach (var legacyBooking in legacyDoctorRows)
        {
            var doctorId = legacyBooking.Patient!.DoctorAssignments
                .Where(x => allowedDoctorIds.Contains(x.DoctorId))
            ...
            {
                legacyBooking.DoctorId = doctorId.Value;
            }
        }
