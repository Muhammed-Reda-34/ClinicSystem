...
export function UsersPage() {
  ...
  const doctorMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setSuccess(ar ? "تم إنشاء حساب الطبيب بنجاح." : "Doctor account created successfully.");
      await refreshUsers();
    },
    ...
    <section className={styles.page}>
      <header className={styles.hero}>
        <div>
          ...
          <p>
            {ar
              ? "إنشاء حسابات الأطباء والسكرتارية والتمريض وربط كل موظف بالطبيب المناسب."
              : "Create doctor, secretary and nurse accounts and manage doctor assignments."}
          </p>
        </div>
        ...
        >
          <div className={styles.formHeader}>
            ...
            <div>
              ...
              <p>{ar ? "بيانات الدخول والتخصص للطبيب الجديد." : "Login and specialty details for the new doctor."}</p>
            </div>
          </div>
          ...
          <small className={styles.hint}>
            {ar ? "8 أحرف على الأقل + حرف كبير + حرف صغير + رقم." : "At least 8 characters with uppercase, lowercase and a number."}
          </small>

          {doctorError && <div className={styles.error}>{doctorError}</div>}

          <button className={styles.primary} disabled={doctorMutation.isPending}>
            {doctorMutation.isPending ? (ar ? "جاري الإنشاء..." : "Creating...") : (ar ? "إنشاء حساب الطبيب" : "Create doctor account")}
          </button>
        </form>
        ...
        >
          <div className={styles.formHeader}>
            ...
            <div>
              ...
              <p>{ar ? "حدد الوظيفة والطبيب أو الأطباء الذين تعمل معهم." : "Choose the role and doctors this staff member works with."}</p>
            </div>
          </div>
          ...
          <small className={styles.hint}>
            {ar ? "بعد الإنشاء يتم تفريغ كل الحقول تلقائيًا." : "All fields are cleared automatically after creation."}
          </small>
          ...
          >
            {staffMutation.isPending ? (ar ? "جاري الإنشاء..." : "Creating...") : (ar ? "إنشاء حساب الموظف" : "Create staff account")}
          </button>
        </form>
      </div>
