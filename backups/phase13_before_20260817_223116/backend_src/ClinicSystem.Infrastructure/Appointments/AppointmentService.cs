...
public sealed class AppointmentService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var appointmentNoShowCount = await _db.Appointments
            .CountAsync(
                a =>
        ...
        var preliminaryNoShowCount = await _db.PreliminaryBookings
            .CountAsync(
                booking =>
                    booking.PatientId == appointment.PatientId
                    && booking.AttendanceStatus
                        == AppointmentAttendanceStatus.NoShow
                    && (
                        patient.BlacklistClearedAtUtc == null
                        || booking.UpdatedAtUtc > patient.BlacklistClearedAtUtc
                    ),
                cancellationToken);

        var noShowCount =
            appointmentNoShowCount
            + preliminaryNoShowCount;
